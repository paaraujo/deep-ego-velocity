...
class RadarDataset(Dataset):
    ...
    def __init__(self, path, map_params, augment_params, other_params):
        ...
        selection = os.listdir(split_path)
        ...
        with open(self.unified_file, 'w') as u:
            ...
            for sequence in selection:
                ...
                with open(sequence, 'rb') as f:
                    # Data is a list(timestamp, ['x','y','z','alpha','beta','doppler','power'], speed)
                    data = pickle.load(f)
                    for timestamp, scans, speed in data:
                        self.offset_dict[self.length] = u.tell()
                        u.write(str([timestamp, scans, speed])+'\n')
    ...
    def _map(self, data):
        ''' data has columns in the following order: ['x','y','z','alpha','beta','doppler','power'] '''
...
class SequenceRadarDataset(Dataset):
    ...
    def __init__(self, path, map_params, augment_params, other_params):
        ...
        for f in files:
            if other_params['sequence'] in f:
                selection.append(f)
        if not selection:
            print(f"Sequence '{other_params['sequence']}' not found in split '{other_params['split']}'.")
