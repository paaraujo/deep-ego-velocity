...
def train(tag):
    ...
    convnet_params = {
        ...
        'groups': 2,
        'in_channels': 40,  
        'channels': [40, 40, 40, 40, 40, 40],  # 0.5 m/pixel
        'input_size': (1, 40, 100, 168)
    }
    linearnet_params = {
    ...
    # tag_weights = '3D_25_Feb_2024_05_16_07'
    # epochs = 0
    ...
    other_params = {'split': 'train', 'dropout': 0.5, 'epochs':0, 'threshold':0, 'device': device}
    ...
    # loss_fn = nn.MSELoss()
    optimizer = optim.SGD(model.parameters(), momentum=0.9, lr=1e-6, weight_decay=1e-5)
...
def test(tag):
    ...
    # Saving results
    map_params = {'px':convnet_params['input_size'][2], 'py':convnet_params['input_size'][3], 'pz':convnet_params['input_size'][1], 'rx':20, 'ry':37 * 2, 'rz':6, 'groups':convnet_params['groups']}
    augment_params = {'augment': False, 'delta_x': 15.0, 'delta_y': 15.0, 'delta_z': 15.0}
...
if __name__ == '__main__':
    tag = '3D_' + datetime.now().strftime("%d_%b_%Y_%H_%M_%S")
    train(tag)
    # tag = '3D_22_Feb_2024_20_52_42'
    # test(tag)