...
def train(tag):
    ...
    convnet_params = {
        ...
        'groups': 1,
        'in_channels': 2,  
        'channels': [64, 32, 32, 16, 8, 4, 2],
        'input_size': (1, 2, 67, 121)
    }
    linearnet_params = {
    ...
    # tag_weights = '2D_FR_23_Feb_2024_05_57_01'
    # epochs = 0
    # checkpoints_path = os.path.join(root, 'checkpoints', 'navinst', tag_weights, 'weights.pt')
    # weights = torch.load(checkpoints_path)
    # model.load_state_dict(weights['model_state_dict'])
    # model.conv.requires_grad_(False)

    # Creating datasets
    map_params = {'px':convnet_params['input_size'][2], 'py':convnet_params['input_size'][3], 'pz':convnet_params['input_size'][1], 'rx':20.0, 'ry':36.3, 'rz':5.3, 'groups':convnet_params['groups']}
    augment_params = {'augment': True, 'delta_x': 15.0, 'delta_y': 15.0, 'delta_z': 15.0}
    other_params = {'split': 'train', 'sensor_id': sensor_id, 'dropout': 0.5, 'epochs':0, 'threshold':50, 'device': device}
    train_set = RadarDataset(os.path.join(root, 'data', 'navinst', 'synced'), map_params, augment_params, other_params)
    # hist(train_set.speeds, bins=14, res=2)

    map_params = {'px':convnet_params['input_size'][2], 'py':convnet_params['input_size'][3], 'pz':convnet_params['input_size'][1], 'rx':20.0, 'ry':36.3, 'rz':5.3, 'groups':convnet_params['groups']}    
    augment_params = {'augment': False, 'delta_x': 15.0, 'delta_y': 15.0, 'delta_z': 15.0}
    other_params = {'split': 'validation', 'sensor_id': sensor_id, 'device': device}
    valid_set = RadarDataset(os.path.join(root, 'data', 'navinst', 'synced'), map_params, augment_params, other_params)
    # hist(valid_set.speeds, bins=14, res=2)

    map_params = {'px':convnet_params['input_size'][2], 'py':convnet_params['input_size'][3], 'pz':convnet_params['input_size'][1], 'rx':20.0, 'ry':36.3, 'rz':5.3, 'groups':convnet_params['groups']}
    augment_params = {'augment': False, 'delta_x': 15.0, 'delta_y': 15.0, 'delta_z': 15.0}
    other_params = {'split': 'test', 'sensor_id': sensor_id, 'device': device}
    test_set = RadarDataset(os.path.join(root, 'data', 'navinst', 'synced'), map_params, augment_params, other_params)
    ...
    checkpoints_path = os.path.join(root, 'checkpoints', 'navinst', tag)
    os.makedirs(checkpoints_path, exist_ok=True)

    # Creating tensorboard log system
    writer = SummaryWriter(os.path.join(root, 'runs', 'navinst', tag))
    ...
    for epoch in range(epochs+1):
        ...
        if valid_loss < best_valid_loss:
            ...
            }, os.path.join(root, 'checkpoints', 'navinst', tag, 'weights.pt'))


        # Previewing test data
        if epoch % 20 == 0:
            path = os.path.join(root, 'checkpoints', 'navinst', tag, 'Images')
...
def test(tag):
    ...
    checkpoints_path = os.path.join(root, 'checkpoints', 'navinst', tag, 'weights.pt')
    weights = torch.load(checkpoints_path)
    model.load_state_dict(weights['model_state_dict'])
    ...
    map_params = {'px':convnet_params['input_size'][2], 'py':convnet_params['input_size'][3], 'pz':convnet_params['input_size'][1], 'rx':20.0, 'ry':36.3, 'rz':5.3, 'groups':convnet_params['groups']}
    augment_params = {'augment': False, 'delta_x': 15.0, 'delta_y': 15.0, 'delta_z': 15.0}
    other_params = {'split': 'test', 'sequence': '2022-09-06-12-55-08', 'sensor_id': sensor_id, 'device': device}
    test_set = SequenceRadarDataset(os.path.join(root, 'data', 'navinst', 'synced'), map_params, augment_params, other_params)
    preview_loader = DataLoader(test_set, batch_size=1, shuffle=False, pin_memory=True, num_workers=4)
    path = os.path.join(root, 'checkpoints', 'navinst', tag, 'sequences', other_params['sequence']) 
...
if __name__ == '__main__':
    tag = '2D_FR_' + datetime.now().strftime("%d_%b_%Y_%H_%M_%S")
    train(tag)
    # tag = '2D_FR_24_Feb_2024_20_59_23'
    # test(tag)