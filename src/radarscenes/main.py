...
def preview(model, loader, path, save=False):
    ...
    if save:
        ...
        timestamps = np.array(timestamps, dtype=np.int64).reshape((-1,1))
...
def train(tag):
    ...
    # tag_weights = 'A_12_Feb_2024_20_27_16'
    # epochs = 100
    # checkpoints_path = os.path.join(root, 'checkpoints', 'radarscenes', tag_weights, 'weights.pt')
    ...
    other_params = {'split': 'train', 'sensor_id': sensor_id, 'in_channels': convnet_params['in_channels'], 'dropout': 0.5, 'epochs':0, 'threshold':50, 'device': device}
    train_set = RadarDataset(os.path.join(root, 'data', 'radarscenes', 'data'), map_params, augment_params, other_params)
    ...
    map_params = {'px':convnet_params['input_size'][2], 'py':convnet_params['input_size'][3], 'rx':100, 'ry':87 * 2}
    augment_params = {'augment': False, 'delta_z': 15.0}
    other_params = {'split': 'test', 'sensor_id': sensor_id, 'in_channels': convnet_params['in_channels'], 'device': device}
    test_set = RadarDataset(os.path.join(root, 'data', 'radarscenes', 'data'), map_params, augment_params, other_params)
    ...
    checkpoints_path = os.path.join(root, 'checkpoints', 'radarscenes', tag)
    os.makedirs(checkpoints_path, exist_ok=True)

    # Creating tensorboard log system
    writer = SummaryWriter(os.path.join(root, 'runs', 'radarscenes', tag))
    ...
    for epoch in range(epochs+1):
        ...
        if valid_loss < best_valid_loss:
            ...
            }, os.path.join(root, 'checkpoints', 'radarscenes', tag, 'weights.pt'))


        # Previewing test data
        if epoch % 20 == 0:
            path = os.path.join(root, 'checkpoints', 'radarscenes', tag, 'Images')
...
def test(tag):
    ...
    checkpoints_path = os.path.join(root, 'checkpoints', 'radarscenes', tag, 'weights.pt')
    weights = torch.load(checkpoints_path)
    model.load_state_dict(weights['model_state_dict'])
